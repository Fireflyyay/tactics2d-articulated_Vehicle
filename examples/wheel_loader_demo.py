...
def main():
    ...
    final_rear_state = wheel_loader.get_rear_axle_state()
    print(f"   最终位置: ({final_rear_state.x:.2f}, {final_rear_state.y:.2f})")
    ...
    rear_positions = [
        (
            wheel_loader.get_rear_axle_state(frame).x,
            wheel_loader.get_rear_axle_state(frame).y,
        )
        for frame in wheel_loader.trajectory.frames
    ]
